The State Space
===============
The cartesian product of the core state space and the dense gird maps
into the dense state space. 
Since this object is growing large and since we want to remain flexible
we split the dense state space in a large number of smaller state spaces. 
The different parts and subparts of the state space are explained below: 
...
--------------------
To create the state space and to store information efficiently we
build simple indices of the objects introduced above.  
In general we call location indices indices if the defining mapping is
injective and keys if the defining mapping is not injective.
...
.. _complex:
A complex key is the basis for `core_key` and `dense_key` it is a
