!*******************************************************************************
!
!   This code is tailored to the optimization problem in the ROBUPY package.
!   All changes required extensive testing against the SLSQP results from the
!   SCIPY package.
!
!   This code was developed and tested to work for a single inequality
!   constraint. Extensions should be straightforward.
!
!*******************************************************************************
...
!************************************************************************
!
      SUBROUTINE slsqp(m, meq, n, x, xl, xu, f, c, g, a, &
                       acc, iter, mode, w, l_w)
!
...
!
      ...
      INTEGER          il, im, ir, is, iter, iu, iv, iw, ix, l_w, l_jw, &
                      la, m, meq, mineq, mode, n, n1
!
      DOUBLE PRECISION acc, a(:,:), c(:), f, g(n+1), &
                      x(n), xl(n), xu(n), w(l_w)

    INTEGER(our_int), ALLOCATABLE :: jw(:)

    ! Cleaning up the interface by using allocatable arrays.
    !
    !
    !
la = MAX(1, m)
...
!
  240 h3 = ZERO
      DO 250 j=1,m
         ...
         h3 = h3 + MAX(-c(j),h1)
  250 CONTINUE
      IF ((ABS(f-f0).LT.acc .OR. dnrm2_(n,s,1).LT.acc) .AND. h3.LT.acc) THEN
            mode = 0
         ELSE
            mode = -1
         ENDIF
...
!
  255 CONTINUE
      IF ((ABS(f-f0).LT.tol .OR. dnrm2_(n,s,1).LT.tol) .AND. h3.LT.tol) THEN
            mode = 0
         ELSE
            mode = 8
         ENDIF
...
!
   ...
   10         IF(h(l).GT.h(lmax)) lmax=l
          IF(diff(hmax+factor*h(lmax),hmax).GT.ZERO)   GOTO 50
   20     lmax=j
          DO 40 l=j,n
              h(l)=ZERO
              DO 30 i=j,m
...
!C END OF LDL
      END
!
      FUNCTION linmin (mode, ax, bx, f, tol)
...
!
      ...
      DOUBLE PRECISION f, tol, a, b, c, d, e, p, q, r, u, v, w, x, m, &
                      fu, fv, fw, fx, eps, tol1, tol2, ZERO, ax, bx, linmin
