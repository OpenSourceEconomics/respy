...
def pyth_evaluate_revised(coeffs_a, coeffs_b, coeffs_edu, coeffs_home,
        shocks_cov,
        is_deterministic, is_interpolated, num_draws_emax, is_ambiguous,
        num_periods, num_points, is_myopic, edu_start, is_debug, measure,
        edu_max, min_idx, delta, level, data_array, num_agents, num_draws_prob,
    ...
    for i in range(num_agents):
        for period in range(num_periods):
            ...
            for s in range(num_draws_prob):
                ...
                if is_working:
                    crit_val_contrib *=  norm.pdf(draws_prob[s, idx], 0.0,
                ...
                tau = 500
                vmax = max(total_payoffs)
                smoothed_values = np.zeros(4)
                smoothed_values[0] = np.exp((total_payoffs[0] - vmax)/tau)
                smoothed_values[1] = np.exp((total_payoffs[1] - vmax)/tau)
                smoothed_values[2] = np.exp((total_payoffs[2] - vmax)/tau)
                smoothed_values[3] = np.exp((total_payoffs[3] - vmax)/tau)

                sumv = sum(smoothed_values)

                prob = smoothed_values[idx] / sumv
                prob *= crit_val_contrib
