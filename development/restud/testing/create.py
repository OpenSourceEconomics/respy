...
for i in range(1000):
    ...
    # The RESTUD code uses the same random draws for the solution and
    # simulation of the model. Thus, the number of
    #
    #
    #
    ...
    with open('in.txt', 'w') as file_:
        ...
        for j in range(4):
            # TODO: GENERALIZE CODE, THIS IS A MAJOR NEXT STEP; SAVE ACCOM
    ...
    py = pd.DataFrame(np.genfromtxt('data.robupy.dat', missing_values = '.')[
                      :, -4:])

    fort = pd.DataFrame(np.genfromtxt('ftest.txt', missing_values = '.')[:, -4:])

    with open('python.dat', 'w') as file_:

        py.to_string(file_, index=False, header=None, na_rep='.')

    with open('fort.dat', 'w') as file_:
