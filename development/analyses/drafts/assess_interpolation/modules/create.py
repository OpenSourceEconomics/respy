...
def assess_approximation(num_agents, states_all, num_periods, mapping_state_idx,
        ...
        delta, periods_emax_full, periods_emax_approximate):
    """ Sample simulation
    """
...
def process_tasks(task):
    ...
    init_dict['AMBIGUITY']['level'] = 0.01
...
# Ensure that fast version of package is available
os.system('./clean')

if 'acropolis' in HOSTNAME:
    # The compiler on acropolis does use other debugging flags and thus no
...
for spec in ['one', 'two', 'three']:
    for solution in ['full', 'approximate']:
        tasks += [(spec, solution)]

# Create the full set of results and run multiprocessing module
num_procs = 6
p = Pool(num_procs)
rslts = p.map(process_tasks, tasks)
...
for spec in ['one', 'two', 'three']:
    ...
    periods_emax = full_rslts.get_attr('periods_emax')
    ...
    failure_counts[spec] = assess_approximation(num_agents, states_all,
