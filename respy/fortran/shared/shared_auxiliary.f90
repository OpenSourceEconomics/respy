...
!------------------------------------------------------------------------------
    ...
    shocks_cholesky(1, :1) = x(34:34)

    shocks_cholesky(2, :2) = x(35:36)

    shocks_cholesky(3, :3) = x(37:39)

    shocks_cholesky(4, :4) = x(40:43)
...
!------------------------------------------------------------------------------
    ...
    optim_paras%coeffs_edu = x(25:30)

    optim_paras%coeffs_home = x(31:33)
    ...
    optim_paras%type_shares = x(44:(44 + num_types - 1))

    optim_paras%type_shifts = zero_dble
    optim_paras%type_shifts(2:, :) =  TRANSPOSE(RESHAPE(x(44 + num_types:num_paras), (/4, num_types  - 1/)))
...
!------------------------------------------------------------------------------
    ...
    x_internal(25:30) = optim_paras%coeffs_edu(:)

    x_internal(31:33) = optim_paras%coeffs_home(:)

    x_internal(34:34) = optim_paras%shocks_cholesky(1, :1)

    x_internal(35:36) = optim_paras%shocks_cholesky(2, :2)

    x_internal(37:39) = optim_paras%shocks_cholesky(3, :3)

    x_internal(40:43) = optim_paras%shocks_cholesky(4, :4)

    x_internal(44:(44 + num_types - 1)) = optim_paras%type_shares(:)

    shifts = PACK(TRANSPOSE(optim_paras%type_shifts), .TRUE.)
    x_internal(44 + num_types:num_paras) = shifts(5:)
