...
def pyth_contributions(periods_rewards_systematic, mapping_state_idx, periods_emax, states_all,
        data_array, periods_draws_prob, tau, edu_spec, num_periods, num_draws_prob,
        num_agents_est, num_obs_agent, num_types, optim_paras):
                ...
                for s in range(num_draws_prob):
                    ...
                    if is_working and (not is_wage_missing):
                        if is_deterministic:
                            ...
                            prob_wage = norm.pdf(draws_stan[idx], 0.0, 1.0) / np.sqrt(shocks_cov[idx, idx])
                    else:
                        prob_wage = 1.0
