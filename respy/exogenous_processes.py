...
def compute_transition_probabilities(
    states, transit_keys, exogenous_grid, optim_paras, dense_key_to_exogenous
):
...
def weight_continuation_values(
    complex, options, continuation_values, transit_key_to_choice_set
):
    ...
    continuation_values_adjusted = {
        continuation_values[int(ftr_key)][:, choice_positons[ftr_key]]
...
def create_continuation_choice_set(
    ...
    for dense_key in continuation_choice_sets:
        out[sense_key] = _get_maximal_choice_set(continuation_choice_sets[dense_key])
    return out
...
def create_transition_objects(
    ...
    reachable_dense_indices = [dense_covariates_to_dense_index[(*static_dense, *exog)]]
